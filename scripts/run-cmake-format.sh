...
status=0
if [ -z ${RAPIDS_LOGGER_ROOT:+PLACEHOLDER} ]; then
    RAPIDS_LOGGER_BUILD_DIR=$(git rev-parse --show-toplevel 2>&1)/cpp/build
    status=$?
else
    RAPIDS_LOGGER_BUILD_DIR=${RAPIDS_LOGGER_ROOT}
fi

if ! [ ${status} -eq 0 ]; then
    if [[ ${RAPIDS_LOGGER_BUILD_DIR} == *"not a git repository"* ]]; then
        echo "This script must be run inside the cudf repository, or the RAPIDS_LOGGER_ROOT environment variable must be set."
    else
        echo "Script failed with unknown error attempting to determine project root:"
        echo ${RAPIDS_LOGGER_BUILD_DIR}
    fi
    exit 1
fi

DEFAULT_FORMAT_FILE_LOCATIONS=(
  "${RAPIDS_LOGGER_BUILD_DIR:-${HOME}}/_deps/rapids-cmake-src/cmake-format-rapids-cmake.json"
  "${RAPIDS_LOGGER_BUILD_DIR:-cpp/build}/latest/_deps/rapids-cmake-src/cmake-format-rapids-cmake.json"
)
